...
type SearchServiceClient interface {
	...
	SearchInOut(ctx context.Context, opts ...grpc.CallOption) (SearchService_SearchInOutClient, error)
}
...
func (c *searchServiceClient) SearchInOut(ctx context.Context, opts ...grpc.CallOption) (SearchService_SearchInOutClient, error) {
	stream, err := c.cc.NewStream(ctx, &SearchService_ServiceDesc.Streams[2], "/person.SearchService/SearchInOut", opts...)
	if err != nil {
		return nil, err
	}
	x := &searchServiceSearchInOutClient{stream}
	return x, nil
}

type SearchService_SearchInOutClient interface {
	Send(*PersonReq) error
	Recv() (*PersonRes, error)
	grpc.ClientStream
}

type searchServiceSearchInOutClient struct {
	grpc.ClientStream
}

func (x *searchServiceSearchInOutClient) Send(m *PersonReq) error {
	return x.ClientStream.SendMsg(m)
}

func (x *searchServiceSearchInOutClient) Recv() (*PersonRes, error) {
	m := new(PersonRes)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type SearchServiceServer interface {
	...
	SearchInOut(SearchService_SearchInOutServer) error
...
func (UnimplementedSearchServiceServer) SearchInOut(SearchService_SearchInOutServer) error {
	return status.Errorf(codes.Unimplemented, "method SearchInOut not implemented")
}
...
func _SearchService_SearchInOut_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(SearchServiceServer).SearchInOut(&searchServiceSearchInOutServer{stream})
}

type SearchService_SearchInOutServer interface {
	Send(*PersonRes) error
	Recv() (*PersonReq, error)
	grpc.ServerStream
}

type searchServiceSearchInOutServer struct {
	grpc.ServerStream
}

func (x *searchServiceSearchInOutServer) Send(m *PersonRes) error {
	return x.ServerStream.SendMsg(m)
}

func (x *searchServiceSearchInOutServer) Recv() (*PersonReq, error) {
	m := new(PersonReq)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
var SearchService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "SearchInOut",
			Handler:       _SearchService_SearchInOut_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
